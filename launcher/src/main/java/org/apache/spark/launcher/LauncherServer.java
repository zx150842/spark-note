 ...
 *
 * 用来在本地监听由lib提交的client的链接请求。每个client都有一个秘钥需要在建立连接时发送给server以
 * 认证和建立session
 *
 ...
 * disconnected).
 *
 * launcher server只监听本地请求，所以不需要访问控制或者加密。这也要求提交的app需要有一个本地进程来与server
 * 通信。在集群模式，这意味着提交应用的client必须在应用运行期间（或在app handle关闭之前）保持存活
 */
