 ...
 *
 * 负责处理来自client的请求，并返回响应。每个handler都有一个相关联的netty channel，并监控与这个channel相连的
 * 流的情况，以便当channel被终止时可以清理与channel连接的流。
 * 到达这里的消息已经被TransportServer处理过了。
 */
public class TransportRequestHandler extends MessageHandler<RequestMessage> {
  ...
  /** 当前节点用来向channel另一端请求的客户端发送响应的client */
  ...
  private void processRpcRequest(final RpcRequest req) {
    try {
      // 在这里使用NettyEnv中的handler实际处理接收到的请求
