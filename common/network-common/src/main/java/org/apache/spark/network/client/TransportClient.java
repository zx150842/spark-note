 ...
 *
 * client发送的request类型：ChunkFetchRequest，StreamRequest，RpcRequest，OneWayMessage
 */
public class TransportClient implements Closeable {
   ...
   *
   * 根据预先约定的streamId向远端请求一个块
   * chunkIndex从0开始，可以对同一个chunkIndex调用多次，虽然有的流不支持这个功能
   * 可以同时发送多个fetchChunk请求，块被保证与他们请求的顺序相同的顺序返回（需要保证只
   * 有一个TransportClient被用来请求块）
   *
   ...
   *
   * 将透明的消息发送给server的rpcHandler。server返回响应或有任何的失败回调函数都会被调用
   *
