 ...
 * executors.
 *
 * 用来读取shuffle blocks并保存到外部服务器上（不是所在的executor）的client。这个方法代替直接从其他
 * executors读取shuffle blocks（通过BlockTransferService），直接读取的缺点是在丢失executor时，上面
 * 的shuffle data也会丢失
 */
public class ExternalShuffleClient extends ShuffleClient {
   ...
   *
   * 将executor注册到外部shuffle server上。这个注册需要告知shuffle server我们在哪里以及如何存储
   * shuffle file
   *
