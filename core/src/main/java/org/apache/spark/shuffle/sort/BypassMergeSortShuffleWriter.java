 ...
 *
 * 这个类根据reduce分区的不同，将记录写入不同的文件，然后将这些文件进行合并，输出一个文件供reducers使用。
 * 记录不会在内存中缓存，这个类基本上与HashShuffleWriter相同，只是他的输出时格式化的，以便IndexShuffleBlockResolver
 * 使用。
 * 这个类对于有大量reduce分区的shuffle不是很高效，因为这个类会为每个分区都打开一个序列化器以及文件。所以只有SortShuffleManager
 * 在如下场景会使用这个类：
 * 1 没有ordering被指定
 * 2 没有aggregator被指定
 * 3 分区数小于bypassMergeThreshold
 *
 * 这个类是externalSorter类的一部分，这里将其单独抽取出来只是为了降低代码复杂度
 */
final class BypassMergeSortShuffleWriter<K, V> extends ShuffleWriter<K, V> {
  ...
  public void write(Iterator<Product2<K, V>> records) throws IOException {
    ...
    // 对每一个partition建立一个文件，所以如果partition多的话，同时会打开很多的文件
