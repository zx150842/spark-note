 ...
 *
 * 外部排序类。
 * 输入的记录会首先被保存到内存的数据页中。当所有记录被插入或当当前线程的shuffle内存达到上限，
 * 则内存中的记录会根据他们的partition id进行排序（使用shuffleInMemorySorter）。排序的结果被
 * 写入到一个单独的输出文件（如果中间多次spill，则存在多个输出文件）。输出文件的格式与最终写
 * 入最终输出文件的格式相同。每个partition的记录以序列化，压缩流的形式写入，并可以以反序列化，
 * 非压缩的形式读出。
 *
 * 与collection包中的ExternalSorter不同，这个sorter不负责合并spill文件。合并操作由调用它的
 * UnsafeShuffleWriter实现，它使用序列化的方式进行合并操作，避免了额外的序列化/反序列化。
 *
 * 这个类集成自MemoryConsumer，内存由TaskMemoryManager管理。记录都会被先写到内存页上，如果内存不足，
 * 则由TaskMemoryManager调用这个类实现的spill方法将记录进行排序后写到文件中
 */
