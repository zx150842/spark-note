...
public class UnsafeShuffleWriter<K, V> extends ShuffleWriter<K, V> {
   ...
   * kernel bugs.
   *
   * 使用Java FileStreams来合并spill file。这个方法要比基于NIO的合并方法mergeSpillsWithTransferTo慢，
   * 所以只在IO压缩编码不支持压缩数据文件中压缩的数据，或用户禁用了transferTo配置，以便来查询内核bug
   *
  ...
  private long[] mergeSpillsWithFileStream(
    ...
    try {
      ...
      for (int partition = 0; partition < numPartitions; partition++) {
        ...
        // 找到每个spill中partition = 当前partition的数据，写入文件。即最终的文件是按照partition排好序的
        for (int i = 0; i < spills.length; i++) {
   ...
   * serialized streams.
   *
   * 使用NIO transferTo方式来合并spill文件。这个方法只在IO压缩编码以及序列化器支持序列化数据
   * 的级联时才是安全的。
   *
   * @return the partition lengths in the merged file.
   */
