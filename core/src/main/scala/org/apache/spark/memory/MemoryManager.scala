 ...
 * internal data across the cluster. There exists one MemoryManager per JVM.
  *
  * 管理execution和storage内存的抽象类
  *
  * execution内存指的是被用来进行洗牌，join，排序，聚合操作时所使用的内存，
  * storage内存指的是用来缓存和在集群中传播数据所使用的内存
  * 每一个JVM有一个MemoryManager
 */
private[spark] abstract class MemoryManager(
   ...
   *
    * 申请N bytes的内存来缓存给定的块，可能会回收已经分配出去的内存
    *
   ...
   * but an older task had a lot of memory already.
    *
    * 给当前的任务分配最多numBytes的execution内存，返回实际分配的内存大小
   */
  private[memory]
   ...
   * sun.misc.Unsafe.
    *
    * 返回使用的内存模型（堆内/堆外），如果不配置spark.memory.offHeap.enabled默认使用堆内模型
   */
   ...
   * and then divide it by a factor of safety.
    *
    * 获取默认的页大小
    * 如果用户不单独使用spark.buffer.pageSize设置页大小，则使用系统计算的默认值（默认值与系统对
    * JVM进程可用的核数，总内存大小相关）
   */
