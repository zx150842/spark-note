 ...
 * tasks was performed by the ShuffleMemoryManager.
  *
  * 被用来记录被任务使用的内存情况。
  * 这个方法保证每个任务都可以获取到合理的内存大小，而不是一些任务直接获取大量的内存而导致其他的任务申请内存时
  * 为了获取足够的内存而频繁的进行刷磁盘操作
  *
  * 如果有N个任务，这个类保证每个任务在被置换到磁盘前可以至少获取到1 / 2N大小的内存，至多可以获取1 / N大小内存。
  * 由于N是动态变化的，我们跟踪active状态的任务并在N改变时，为等待的任务重新计算1 / 2N和 1 / N。这是通过对可变
  * 状态增加同步以及wait和notifyAll状态来将变化通知给调用者。
 *
...
private[memory] class ExecutionMemoryPool(
   ...
   * but an older task had a lot of memory already.
    *
    * 为每个给定的任务分配内存，返回实际分配给任务的内存，如果不能分配内存返回0
    *
    * 这个方法在某些场景下会阻塞直到获取到足够的空闲内存（如当任务数量增多，而老任务已经占据大量内存时，新来的任务不能获取
    * 至少1/2N的内存），来保证每个任务都有机会获取至少execution总内存1/2N（其中N表示活跃的任务）。
   *
  ...
  private[memory] def acquireMemory(
    ...
    while (true) {
      ...
      // 首先从storage中释放足够的内存（直接获取storage空闲内存 || 收回之前storage借出的内存）
      ...
      // 如果可以分配的内存<任务需要的内存 && 一共分配给任务的内存<任务最少可分配的内存，则阻塞任务，直到其他任务来释放内存
