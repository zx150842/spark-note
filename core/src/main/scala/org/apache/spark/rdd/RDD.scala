...
abstract class RDD[T: ClassTag](
  ...
  private def persist(newLevel: StorageLevel, allowOverride: Boolean): this.type = {
    ...
    // 如果当前的RDD是第一次被缓存，则将RDD注册到SparkContext中以便之后的清理和统计
  ...
  // spark的子rdd如果已经checkpoint了，则默认不checkpoint标记要checkpoint的父rdd
   ...
   * doCheckpoint() is called recursively on the parent RDDs.
    *
    * 这个方法会从finalRdd开始从后向前寻找距离finalRdd最近的调用了checkpoint方法的rdd，并在这个rdd上
    * 调用runJob方法，将这个rdd内容checkpoint。如果存在父子rdd都调用了checkpoint方法，当前spark默认只
    * checkpoint最近的rdd（即子rdd的数据）
   */
      ...
      if (!doCheckpointCalled) {
        ...
        if (checkpointData.isDefined) {
          if (checkpointAllMarkedAncestors) {
            ...
            // 对父rdd进行checkpoint（也会调用runJob方法）
