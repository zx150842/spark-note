...
class CoGroupedRDD[K: ClassTag](
  ...
  override def compute(s: Partition, context: TaskContext): Iterator[(K, Array[Iterable[_]])] = {
    ...
    // zipWithIndex 遍历dependency并返回每个value和其对应的index
  ...
  private def createExternalMap(numRdds: Int)
    ...
    // 这里的value就是map的value即compute传入的CoGroupValue，
    // value._2=CoGroupValue.depNum，value._1=CoGroupValue.it._2，即iterator value
    // 即建立一个大小为依赖rdd数量的数组array，遍历依赖的所有的rdd，将每个rdd对应的value
    // 放入rdd在数组中对应的位置(array[rdd.number])，即cogroup依赖两个rdd，thisRdd和otherRdd，
    // 则thisRdd的value会放入array[0]中，otherRdd的value会放入array[1]中
