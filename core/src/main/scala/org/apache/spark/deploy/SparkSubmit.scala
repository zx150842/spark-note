...
object SparkSubmit {
  ...
  private[deploy] def prepareSubmitEnvironment(args: SparkSubmitArguments)
    ...
    // 如果是client模式，则直接在当前运行submit脚本的机器作为driver，运行app的main方法
    ...
    // 如果是standalone的cluster模式，如果使用rest client提交app，则调用RestSubmissionClient
    // 否则调用Client提交app到集群，然后集群分配一台机器作为driver，再调用app的main方法
