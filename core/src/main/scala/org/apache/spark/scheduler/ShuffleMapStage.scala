 ...
 * there can be multiple ActiveJobs trying to compute the same shuffle map stage.
  *
  * ShuffleMapStages是用来为shuffle提供数据的DAG stage。ShuffleMapStages在shuffle操作前被创建，并
  * 可能在shuffle前包含多个并发操作（如map和filter）。当执行时，ShuffleMapStages会将map结果保存到
  * 文件中从而可以被之后的reduce任务获取。ShuffleDep描述了每个ShuffleMapStage中的shuffle依赖。变量
  * outputLocs和numAvailableOutputs记录了有多少的map输出已经就绪。
  *
  * ShuffleMapStages可以与job一样被独立提交（使用DAGScheduler.submitMapStage）。对每一个stage，提交
  * 这些stage的job被记录在mapStageJobs成员变量中。要注意的是可能存在多个job同时尝试计算相同的shuffle
  * map stage。
 */
private[spark] class ShuffleMapStage(
   ...
   * This should be kept consistent as `outputLocs.filter(!_.isEmpty).size`.
    *
    * 记录包含shuffle输出文件的分区数量。当数量=rdd分区数量时，说明map stage已经就绪。
   */
  ...
  // 将shuffle partition map任务的执行状态更新到outputLocs中
