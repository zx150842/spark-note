 ...
 *
  * TaskSetManager负责在TaskSchedulerImpl中调度一个TaskSet中的所有任务。这个类跟踪每个任务，以及失败重试
  * （重试最大次数由maxTaskFailure指定），并通过延迟调度来实现位置感知调度。这个类的主要接口是resourceOffer，
  * 用来询问TaskSet中的任务是否想要在一个节点运行，并更新任务状态，向TaskManager汇报任务执行状态。
  *
...
private[spark] class TaskSetManager(
   ...
   * it just failed.
   *
   * 加入黑名单的时间。如果executor意外终止导致任务执行是啊比，则会将executor加入黑名单，
   * 不在分配任务给他。
   * 如果一个executor挂掉或处于无效状态，由于会重试任务，driver不会第一时间发现。如果设置
   * 了EXECUTOR_TASK_BLACKLIST_TIMEOUT，则会阻止在executor上的任务失败一次后就重新分配任务
   */
  ...
  // 首先，定义TaskSetManager进入zombie状态——TaskSet中至少有一个task运行完成或者整个taskset被抛弃；
  // zombie状态会一直保持到所有的task都执行完成；之所以让TaskSetManager处于zombie状态，是因为这时可
  // 以跟踪所有正在运行的task。
  var isZombie = false
  ...
  // 将task按照preferredLocations不同划分到不同的队列中
   ...
   * Respond to an offer of a single executor from the scheduler by finding a task
    *
    * 根据传入的execId和host找到可以调度到execId和host的task
   *
   ...
   * we don't scan the whole task set. It might also help to make this sorted by launch time.
    *
    * 计算taskSet中已完成的任务所耗时间的中值，将执行时间超过中值的未完成任务保存到speculatableTasks
   */
  override def checkSpeculatableTasks(minTimeToSpeculation: Int): Boolean = {
    ...
    if (tasksSuccessful >= minFinishedForSpeculation && tasksSuccessful > 0) {
      ...
      for ((tid, info) <- taskInfos) {
        val index = info.index
        // 如果任务未完成 && 任务没有运行的副本 && 任务运行时间 > 阈值 && speculatableTasks不包含当前任务
        // 则将当前任务加入speculatableTasks
   ...
   * added to queues using addPendingTask.
    *
    * 计算当前TaskSet中使用的locality levels。这里默认所有的任务已经使用addPendingTask方法添加到任务队列中
   *
   */
