 ...
 * and divides the task output to multiple buckets (based on the task's partitioner).
  *
  * 一个spark job包含一个或多个stage。job中最后的一个stage包含多个ResultTask，job中其他的stage包含
  * 多个ShuffleMapTask。ResultTask执行任务并将任务的输出结果上报给driver应用。ShuffleMapTask执行任
  * 务并将任务结果根据任务分区策略输出到多个桶中。
 *
