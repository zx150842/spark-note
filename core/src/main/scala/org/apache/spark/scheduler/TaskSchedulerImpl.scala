 ...
 * we are holding a lock on ourselves.
  *
  * 通过SchedulerBackend对不同类型的集群适配，并调度任务。TaskScheduler还可以通过配置isLocal=true
  * 在本地运行。TaskScheduler处理一些基本的逻辑，如决定job的调度顺序，唤醒并执行潜在的任务。
 */
  ...
  extends TaskScheduler with Logging
{
  ...
  // 在任务运行至少MIN_TIME_TO_SPECULATION之后，才能将任务的副本提交到集群执行。这个配置是为了避免
  // 在短时间内提交多个相同的任务
   ...
   * that tasks are balanced across the cluster.
    *
    * 由cluster manager调用来向slave提供资源。我们循环的将任务填充到每个节点上，来保证任务可以在集群
    * 上负载平均。
   */
