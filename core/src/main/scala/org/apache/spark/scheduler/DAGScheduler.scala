 ...
 * files from previous stages), though it may fail if this data becomes unavailable.
  *
  * DAGScheduler是面向stage调度的高层次实现。对每一个job，都会计算生成一个由多个stage组成的DAG，
  *并跟踪RDD和stage输出，找到运行job的最小调度策略。之后会将stage以TaskSets的形式向TaskScheduler提交，
  * 由TaskScheduler负责将这些任务提交到集群执行。一个TaskSet包含全部的独立任务，每个任务所依赖的数据
  * 就绪时就可以立刻执行，当依赖的数据不可用时任务会失败。
 *
 ...
 * various RDDs (MappedRDD, FilteredRDD, etc).
  *
  * spark stage以RDD图中shuffle操作为分割。RDD中的窄依赖，例如map和filter，会最终转换为一个stage上的
  * 一系列的任务，但是对于shuffle依赖的操作需要多个stage（一个stage用来将结果写入一系列的map输出文件，
  * 另一个stage负责在map文件都输出后读取这些文件）。最终每个stage都只会含有其他stage的shuffle依赖，以及
  * 依赖中的计算操作。实际的计算操作由RDD.compute函数完成。
 *
 ...
 * a small number of times before cancelling the whole stage.
  *
  * 另外，DAGScheduler根据当前缓存状态，决定每个任务首选的运行位置，并把位置信息传递给底层的TaskScheduler。
  * DAGScheduler还会处理由于shuffle输出文件丢失而引起的错误，如果发生此类错误，老stage可能会被重新提交。
  * 而stage中不是由于shuffle输出文件丢失而产生的错误直接由TaskScheduler处理。TaskScheduler会重试任务几次，
  * 如果失败会拒绝整个stage。
 *
 ...
 *    redoing the map side of a shuffle.
  *
  *    DAGScheduler会记录哪些RDD被缓存了，来避免重新计算。同样的，DAGScheduler也会记录哪些shuffle map
  *    stage已经输出了map文件，从而避免重新执行shuffle map。
 *
 *  - Preferred locations: the DAGScheduler also computes where to run each task in a stage based
 *    on the preferred locations of its underlying RDDs, or the location of cached or shuffle data.
  *
  *    DAGScheduler会根据任务依赖的RDD或者shuffle数据缓存的位置来决定任务在哪个节点执行。
 *
 *  - Cleanup: all data structures are cleared when the running jobs that depend on them finish,
 *    to prevent memory leaks in a long-running application.
  *
  *    当任务执行完毕后，任务执行过程创建的所有数据结构都会被清除来避免发生内存溢出。
 *
 ...
 * events received in the correct Stage object.
  *
  * 为了从失败中恢复，同一个stage可能会被运行多次，这个被称为attempts。如果TaskScheduler报告其管理的
  * 任务所依赖的上一个stage输出的map文件丢失，则DAGScheduler需要重新提交丢失数据的stage。这个通过
  * FetchFailed CompletionEvent或ExecutorLost event来探测。同时DAGScheduler在重新提交stage之前会等待
  * 一小段时间来看是否还能收到其他节点或任务失败信息，之后将这些丢失的stage一同提交。我们需要创建我们
  * 之前清理了的stage，因为来自老的attempt stage的任务可能还在运行，所以必须注意将获取的event信息与
  * 正确的stage对应。
 *
...
class DAGScheduler(
   ...
   * Create a ResultStage associated with the provided jobId.
    *
    * 为指定的job创建一个ResultStage（最终的stage）
   */
   ...
   * This function is scheduler-visible for the purpose of unit testing.
    ...
    * 这个方法不会返回更远距离的祖先。如C有一个在B上的shuffle依赖，B有一个shuffle依赖于A，调用rdd C
    * 只会返回B<-- C上的依赖
   */
   ...
   * they arrive.
    *
    * 运行给定RDD上job，并将结果使用传入的resultHandler返回
   *
   * @param rdd target RDD to run tasks on
   * @param func a function to run on each partition of the RDD
   * @param partitions set of partitions to run on; some jobs may not want to compute on all
  ...
  def runJob[T, U](
      ...
      partitions: Seq[Int],
      callSite: CallSite,
      resultHandler: (Int, U) => Unit, // partitionNum -> result
  ...
  /**
    ...
    * @param jobId
    * @param finalRDD
    * @param func // 用户定义的处理函数,见RDD类的action方法
    * @param partitions
    * @param callSite
    * @param listener
    * @param properties
   ...
   * modify the scheduler's internal state. Use taskEnded() to post a task end event from outside.
    *
    * 当任务成功执行后，TaskScheduler会调用DAGScheduler的taskEnded方法回传任务运行状态。taskEnded方法
    * 会将任务运行结果封装为CompleteEvent放入eventLoop中，之后由后台线程取出并调用handleTaskCompletion方法执行。
   */
