...
private[netty] class NettyRpcEnv(
  ...
  // RpcEnv用来分发消息到对应RpcEndpoint的分发器
  ...
  /**
    * 向远端endpoint发送消息
    * @param receiver 远端的rpcEndpointRef
    * @param message
    */
  private def postToOutbox(receiver: NettyRpcEndpointRef, message: OutboxMessage): Unit = {
    ...
    if (receiver.client != null) {
      ...
      // 否则通过outbox发送消息（outbox.send方法封装了与远端建立连接的逻辑）
      val targetOutbox = {
...
private[rpc] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    // 只在driver上启动NettyRpcEnv的 TransportServer
    if (!config.clientMode) {
      ...
      try {
        // 借贷模式调用startNettyRpcEnv，启动TransportServer
...
private[netty] class NettyRpcHandler(
  ...
  private def internalReceive(client: TransportClient, message: ByteBuffer): RequestMessage = {
    ...
    if (requestMessage.senderAddress == null) {
      ...
      // 如果客户端是新加入的，需要向所有在RpcEnv注册的endpoint广播此事件
