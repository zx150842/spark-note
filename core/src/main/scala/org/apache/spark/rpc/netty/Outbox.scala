...
private[netty] class Outbox(nettyEnv: NettyRpcEnv, val address: RpcAddress) {
   ...
   * [[Outbox]] is stopped, the sender will be notified with a [[SparkException]].
    * 发送消息，如果没有可用的链接，暂时保存消息并新建一个链接。如果Outbox被终止了，发送者会收到SparkException
   */
  ...
  private def launchConnectTask(): Unit = {
    // connectFuture表示connect task，如果有线程正在建立链接则connectFuture不为空，否则为空
    connectFuture = nettyEnv.clientConnectionExecutor.submit(new Callable[Unit] {
      ...
      override def call(): Unit = {
        ...
        // 成功建立client，则将connectFuture清除，表示没有connect task
        outbox.synchronized { connectFuture = null }
        ...
        // 主动发送一次outbox里面的消息，否则可能要等到下次有消息到来时才能发送之前的消息
        drainOutbox()
      }
    })
  }
