...
private[storage] trait BlockEvictionHandler {
   ...
   *
    * 从内存中删除一个块，如果配置了STORE_DISK，则将块保存到磁盘上。这个方法会在内存不能保存新来的
    * 块，需要释放空间时使用
    * 如果data不需要保存到磁盘上，则无需创建data
    * 调用者在调用这个方法之前必须已经持有块的写锁，这个方法不会释放写锁
    *
 ...
 * serialized ByteBuffers.
  *
  * 在内存中保存存储的块，内存块或者被表示为非序列化的java对象数组，或者是序列化的buffer
 */
private[spark] class MemoryStore(
  ...
  // 所有涉及到内存分配改变的方法，包括添加块，删除块，获取/释放unroll内存都必须在memoryManager
  // 对象上同步

  // 实际保存内存中所有块的map blockId -> memoryEntry
   ...
   *
    * 将size大小的bytes添加到内存中（如果有足够的内存则添加成功，否则添加失败）
    *
   ...
   * so we won't acquire more memory than is actually needed to store the block.
    *
    * 尝试将给定的非序列化块以非序列化的形式保存到内存中。为了防止由于放入的数据块过大导致的内存
    * 溢出，这个方法在遍历块中数据时，定期的检测是否还有足够的unroll内存，如果不足则向storage申
    * 请内存（这个过程不会真正保存数据，只是保存数据的预估大小）。如果最终计算出数据块的大小可
    * 以保存到内存中，则表明storage内存充足，直接从storage中扣除这部分内存数值，然后这时才真正
    * 将数据块保存到内存中，这个方法在真正保存内存之前都是内存数值的计算，并不会真正申请内存，
    * 所以这个方法不会申请比实际需要内存更多的内存来存储块。
   *
  ...
  private[storage] def putIteratorAsValues[T](
    ...
    // 如果成功从storage memory中获取展开的内存，则更新当前任务使用的展开内存大小
    if (!keepUnrolling) {
    ...
    while (values.hasNext && keepUnrolling) {
      vector += values.next()
      // 每读取memoryCheckPeriod个元素后，检查一下内存是否足够
      if (elementsUnrolled % memoryCheckPeriod == 0) {
        // If our vector's size has exceeded the threshold, request more memory
        // 如果当前读入的数据预估大小超过了展开内存的大小，则扩容
        val currentSize = vector.estimateSize()
        if (currentSize >= memoryThreshold) {
          // 一次多申请一些空间
          val amountToRequest = (currentSize * memoryGrowthFactor - memoryThreshold).toLong
          // 尝试再申请amountToRequest大小的空间
          keepUnrolling =
            reserveUnrollMemoryForThisTask(blockId, amountToRequest, MemoryMode.ON_HEAP)
          // 申请成功则更新使用的展开内存总量，如果失败则退出循环
          if (keepUnrolling) {
            unrollMemoryUsedByThisBlock += amountToRequest
          }
          // New threshold is currentSize * memoryGrowthFactor
          // 不论申请是否成功都更新memoryThreshold
    ...
    // 到这如果keepUnrolling还是true，则说明成功分配了所有的unroll需要的内存
    if (keepUnrolling) {
      // We successfully unrolled the entirety of this block
      val arrayValues = vector.toArray
      vector = null
      // 将values包装成memoryEntry
      val entry =
  ...
  /**
    ...
    * 删除内存块流程：
    * 1 删除entries中对应的项
    * 2 如果块是以buffer形式保存的，清空buffer
    * 3 将释放的空间加回到storageMemory上
   ...
   *
    * 尝试移除块来释放内存空间以存储新块
    * 如果新块大小大于内存大小，或替换的是同一个RDD的块，则移除失败
    *
  ...
  private[spark] def evictBlocksToFreeSpace(
    ...
    memoryManager.synchronized {
      ...
      entries.synchronized {
        ...
        // 如果申请的空间大于内存可用空间，并且entries里面还有块，
        // 则尝试将entries中的块移除来释放内存空间
        while (freedMemory < space && iterator.hasNext) {
          val pair = iterator.next()
          val blockId = pair.getKey
          val entry = pair.getValue
          // 判断当前entry的块是否可以被移除：
          // 块的内存模式与申请的内存模式相同 && 块所在rdd != 申请块所在rdd
          if (blockIsEvictable(blockId, entry)) {
            // We don't want to evict blocks which are currently being read, so we need to obtain
            // an exclusive write lock on blocks which are candidates for eviction. We perform a
            // non-blocking "tryLock" here in order to ignore blocks which are locked for reading:
            // 为防止移除正在读的块，这里需要加写锁，lockForWriting方法为非阻塞方法，如果获取不到
            // 写锁，则直接跳过
      ...
      def dropBlock[T](blockId: BlockId, entry: MemoryEntry[T]): Unit = {
        ...
        // 调用blockEvictionHandler移除内存块（刷磁盘，最终调用remove方法删entries）
   ...
   *
    * 尝试从storage memory中获取每个任务保留的展开给定块的内存
  ...
  def reserveUnrollMemoryForThisTask(
    ...
    memoryManager.synchronized {
      ...
      if (success) {
        ...
        // 如果获取成功则将获取的内存大小保存在unrollMemoryMap中
   ...
   * If the amount is not specified, remove the current task's allocation altogether.
    *
    * 释放unroll memory，同时更新unrollMemoryMap大小，如果unrollMemoryMap[taskAttemptId]=0，
    * 则删除map中对应的项
   */
