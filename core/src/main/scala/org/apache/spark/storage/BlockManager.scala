 ...
 * Note that [[initialize()]] must be called before the BlockManager is usable.
  *
  * 在每个节点（driver和executor）都运行的管理器，用来提供本地/远端写入和接收块并保存到多种存储介质中
  * （内存，磁盘，非堆存储）
 */
private[spark] class BlockManager(
  ...
  // 配置是否使用外部的shuffle service
  private[spark] val externalShuffleServiceEnabled =
    conf.getBoolean("spark.shuffle.service.enabled", false)

  // 磁盘块管理器，管理blockId和磁盘上file的映射关系
  ...
  // 虽然storage memory的大小会变，但是这里只用来做log和report，所以没改
  ...
  // 用来处理executor shuffle文件的管理器地址，这个管理器可以是外部的服务，也可以是我们自己的executor
  // BlockManager
  private[spark] var shuffleServerId: BlockManagerId = _

  // 用于从其他executor获取block以及将本地block暴露给其他的executor
  ...
  // blockManager尝试获取block的最大次数，超过这个次数之后需要从driver重新获取block的位置
  private val maxFailuresBeforeLocationRefresh =
    conf.getInt("spark.block.failures.beforeLocationRefresh", 5)

  // 生成当前节点的endpoint，当连接到master后，供endpointref所指向
   ...
   * will be made then.
    *
    * 重新将所有的块上报给driver。当我们被BlockManager移除后恢复，或正在恢复executor crash后在磁盘的块，
    * 需要重新上报所管理的块。
    *
    * 这个方法特意被设计为失败也不抛出异常。如果上传失败，则失败状态会在下一次心跳或新块注册时，会重新注册
    * 所有的块
   */
  private def reportAllBlocks(): Unit = {
    logInfo(s"Reporting ${blockInfoManager.size} blocks to the master.")
    // 遍历当前blockManager所管理的全部块
   ...
   * Note that this method must be called without any BlockInfo locks held.
    *
    * 重新向master注册blockManager并将拥有的block报告给master。这个方法会被心跳线程在发现没有注册过blockManager时调用
   */
  def reregister(): Unit = {
   ...
   * cannot be read successfully.
    *
    * 获取本地块数据的接口，如果不能找到块或不能成功读取块数据则抛出异常
   */
  override def getBlockData(blockId: BlockId): ManagedBuffer = {
    // 如果块是shuffle块，则直接从shuffleManager中取(因为shuffle的文件都在磁盘上)
  ...
  def getMatchingBlockIds(filter: BlockId => Boolean): Seq[BlockId] = {
    ...
    // 获取blockInfoManager和diskBlockManger上全部的block，进行过滤
   ...
   * the slave needs to re-register.
    *
    * 实际上报UpdateBlockInfo消息。master返回true表示块信息成功被保存，false表示slave需要重新注册
   */
   ...
   * and the updated in-memory and on-disk sizes.
    *
    * 返回更新后的块存储状态以及块id。如果块被移除出内存并很可能添加到磁盘，返回新的存储等级，并
    * 更新内存和磁盘的使用量
   */
   ...
   * Releases the read lock upon exception; keeps the read lock upon successful return.
    *
    * 从本地读取块：
    * 如果块以非序列化存储，则首先尝试从磁盘取，取不到再在内存中取，并序列化
    * 如果块以序列化存储，则首先尝试从内存中取，取不到再在磁盘中取，并缓存到内存中
    * 如果取不到，则抛出异常
   */
   ...
   * Get block from remote block managers as serialized bytes.
    * 从远端的blockManager获取序列化的块
    * 首先从master上获取blockId所在块的位置（可能有多个）
    * 遍历每个位置，尝试从中获取块，如果失败则遍历下一个位置
   */
   ...
   * automatically be freed once the result's `data` iterator is fully consumed.
    *
    * 获取blockId的块，首先尝试从本地获取，如果本地没有找到则尝试从远端获取
   */
  def get(blockId: BlockId): Option[BlockResult] = {
    ...
    // 远端获取时直接通过网络请求一次获取到blockId的数据，并返回（这里没有进行持久化存储）
   ...
   * to compute the block, persist it, and return its values.
    *
    * 查询block是否存在，如果不存在，则调用makeIterator方法计算block并持久化block
   *
   ...
   * cases.
    *
    * 获取可以直接将数据写入磁盘的writer，写入的块会被直接添加到文件中
   */
  def getDiskWriter(
      blockId: BlockId,
      file: File,
