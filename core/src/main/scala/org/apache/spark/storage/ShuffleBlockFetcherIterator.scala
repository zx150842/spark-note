 ...
 *
  * 用来获取多个数据块。对于本地的数据块，直接从本地的BlockManager获取，对于远端的数据块，通过
  * BlockTransferService获取
  *
  * 对于远端数据块的获取，限制了内存中最多没有被遍历的大小。每次只能最多获取最多内存大小的数据，当
  * 数据被遍历后，才能继续从远端获取数据。对于本地的数据块数据，由于只是持有一个引用，所以没有内存
  * 限制。
  *
...
final class ShuffleBlockFetcherIterator(
   ...
   * the number of bytes in flight is limited to maxBytesInFlight.
    *
    * 获取数据块的请求队列。使用这个队列，实现控制内存大小maxBytesInFlight
   */
  ...
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[FetchRequest] = {
    ...
    // 将远端请求使用的内存大小限制在maxBytesInFlight / 5，从而允许最多同时从5个节点获取数据，
    // 从而避免被一个节点阻塞在读取数据块上
    ...
    for ((address, blockInfos) <- blocksByAddress) {
      ...
      // 将本地数据块保存到localBlocks数组中，剔除大小为0的数据块
      if (address.executorId == blockManager.blockManagerId.executorId) {
        ...
        while (iterator.hasNext) {
          ...
          // 如果当前需要从远端获取数据块的大小大于规定的上限，则生成一个请求
   ...
   * track in-memory are the ManagedBuffer references themselves.
    *
    * 获取本地数据块，这个方法没有内存上限的限制，可以与获取远端数据块一同进行。因为获取本地数据块
    * 在创建inputStream时是懒加载的，这里只是存了一个ref
   */
