 ...
 * (driver and executor) use different HashMap to store its metadata.
  *
  * 记录每个stage map输出文件位置的类。对于driver和executor分别继承自这个类，使用不同的
  * hashmap来保存元数据
 */
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
  ...
  // 记录当前正在被executor请求获取的map output
   ...
   * throw a SparkException if this fails.
    *
    * 使用trackerEndpoint来从远端获取mapStatus信息
   */
   ...
   * endPartition is excluded from the range).
    *
    * 被executor调用来获取一系列map output partition的URI和大小
   *
   * @return A sequence of 2-item tuples, where the first item in the tuple is a BlockManagerId,
   *         and the second item is a sequence of (shuffle block id, shuffle block size) tuples
   *         describing the shuffle blocks that are stored at that block manager.
   ...
   * on this array when reading it, because on the driver, we may be changing it in place.
    *
    * 尝试从本地货远端获取给定shuffle id的mapStatus
   *
  ...
  private def getStatuses(shuffleId: Int): Array[MapStatus] = {
    ...
    if (statuses == null) {
      ...
      fetching.synchronized {
        ...
        // 如果有其他的executor在获取mapStatus，则这里等待
        ...
        // 到这里fetchedStatus还=null，则当前线程需要发送请求来获取mapStatus
...
private[spark] class MapOutputTrackerMaster(conf: SparkConf,
        ...
        while (true) {
          try {
            ...
            logDebug("Handling request to send map output locations for shuffle " + shuffleId +
              " to " + hostPort)
            // driver根据shuffleId获取mapOutputStatuses
            val mapOutputStatuses = getSerializedMapOutputStatuses(shuffleId)
            // 响应executor请求mapOutputStatus
   ...
   * i.e. the nodes that the most outputs for that partition are on.
    *
    * 返回给定的shuffle中首选的运行map output分区的地址。例如包含分区大多数output文件的节点
   *
...
private[spark] object MapOutputTracker extends Logging {
  ...
  // 将map的输出地址序列化为高效的形式，便于传输到reduce tassk
