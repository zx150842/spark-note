 ...
 *  - A function that is used to generate an RDD after each time interval
  *
  *  离散数据流（DStream），是Spark Streaming中的基础抽象类，将一个持续流表示为一系列的RDD。
  *  DStream可以从实时的数据源创建（如TCP socket，Kafka，Flume等），也可以从其他已存在的DStream
  *  转换而来（如map，window，reduceByKeyAndWindow操作）。当一个Spark Streaming程序运行时，每个
  *  DStream都会周期性的生成一个RDD，RDD或者来自实时的数据或者来自父DStream所生成的RDD
 */
...
abstract class DStream[T: ClassTag] (
   ...
   * or compute-and-cache it.
    *
    * 获取当前时段的RDD，从缓存中获取或计算并缓存
   */
  private[streaming] final def getOrCompute(time: Time): Option[RDD[T]] = {
    // If RDD was already generated, then retrieve it from HashMap,
    // or else compute the RDD
    // 如果当前时段已经生成过RDD，则直接返回
   ...
   * to generate their own jobs.
    *
    * 对当前批次生成一个SparkStreaming job。
   */
  private[streaming] def generateJob(time: Time): Option[Job] = {
    getOrCompute(time) match {
      // 将rdd包装为job并返回（加入当前批次time，以及jobFunc）
      case Some(rdd) =>
        // spark提交job后最终会在JobScheduler中调用job.run()，job.run最终会调用
        // jobFunc来执行。即最终调用context.sparkContext.runJob提交job
        val jobFunc = () => {
   ...
   * 'this' DStream will be registered as an output stream and therefore materialized.
    *
    * 当调用foreach操作时，由于这是一个output操作，所以这个DStream会被注册为output stream，
    * 类似为core中的ResultStage，作为回溯生成RDD DAG的起点
    *
   ...
   * DStream will be generated.
    *
    * 将当前的streaming注册为output stream。这保证了这个DStream的RDD会被创建
   */
