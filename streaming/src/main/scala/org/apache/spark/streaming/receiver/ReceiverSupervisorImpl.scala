 ...
 * object that is used to divide the received data stream into blocks of data.
  *
  * ReceiverSupervisor的实现类，用来提供处理receiver获取的数据的所有需要的方法。ReceiverSupervisor
  * 创建一个BlockGenerator实例来将收到的数据流切分成spark的数据块
 */
private[streaming] class ReceiverSupervisorImpl(
  ...
  // 保存的driver tracker的endpoint
  private val trackerEndpoint = RpcUtils.makeDriverRef("ReceiverTracker", env.conf, env.rpcEnv)

  /** RpcEndpointRef for receiving messages from the ReceiverTracker in the driver */
  // 用来接收来自driver ReceiverTracker的消息
  ...
  // 使用默认的BlockGeneratorListener创建一个默认的BlockGenerator
  ...
  // 对于单个put的数据，首先将其缓存到BlockGenerator中，然后批量合成一个block
  ...
  // 将receiver收到的经过BlockGenerator包装的数据通过receivedBlockHandler保存到BlockManager中
  // 并向driver汇报block信息
  ...
  /**
    * 停止receiver，向driver节点注销receiver（driver中具体响应方法在ReceiverTracker中）
    *
    * @param message
    * @param error
    */
