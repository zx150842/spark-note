 ...
 *
  * 这个类管理execution上的receiver上的ReceiverInputDStream。这个类的实例需要在input stream被添加后，
  * 以及StreamingContext.start被调用后在实例化。因为在初始化时需要input stream的最终集合
  *
...
class ReceiverTracker(ssc: StreamingContext, skipReceiverLaunch: Boolean = false) extends Logging {
  ...
  // 从DStreamGraph获取所有的receiver input
   ...
   * It's only accessed in ReceiverTrackerEndpoint.
    *
    * 记录所有receiver的信息。receiverId -> receiverInfo
   */
  ...
  // 启动endpoint以及receiver线程
  def start(): Unit = synchronized {
    if (isTrackerStarted) {
      throw new SparkException("ReceiverTracker already started")
    }

    // 如果inputStream中包含receiverInputStream
  ...
  private def registerReceiver(
    ...
    // 获取之前分配给receiver可以执行操作的executor列表
    ...
    // 判断当前来注册的executor是否在可执行操作的executor列表中
  ...
  /**
    * 将receiverId和scheduledLocations包装为ReceiverTrackingInfo并保存到receiverTrackingInfos
    *
    * @param receiverId
  ...
  private def getExecutors: Seq[ExecutorCacheTaskLocation] = {
    if (ssc.sc.isLocal) {
      ...
      // 根据blockManger获取host和executorId
   ...
   * "spark.scheduler.maxRegisteredResourcesWaitingTime" rather than running a dummy job.
    *
    * 使用dummy spark job来保证所有的slave都已经注册了。这个方法避免所有的receiver都被分配到
    * 相同的节点执行
   */
   ...
   * worker nodes as a parallel collection, and runs them.
    * 从ReceiverInputDStream获取receiver，并将receiver分发到不同的work节点并发执行
   */
  private def launchReceivers(): Unit = {
    // 从ReceiverInputStream中获取每个stream对应的receiver
  ...
  private class ReceiverTrackerEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint {
    ...
    override def receive: PartialFunction[Any, Unit] = {
      ...
      case StartAllReceivers(receivers) =>
        // 根据调度策略，获取receiver在集群上的executors集合。返回streamId到executors的映射
        val scheduledLocations = schedulingPolicy.scheduleReceivers(receivers, getExecutors)
        for (receiver <- receivers) {
          // 获取streamId对应的executor集合
          val executors = scheduledLocations(receiver.streamId)
          // 更新运行receiver的executor
          updateReceiverScheduledExecutors(receiver.streamId, executors)
          // 更新receiverPreferredLocations缓存
    ...
    private def startReceiver(
      ...
      // 具体执行逻辑（可以看做是action方法）
      // 主要用来从外部数据源拉来数据后，调用supervisor方法保存到spark的BlockManager中
      val startReceiverFunc: Iterator[Receiver[_]] => Unit =
        (iterator: Iterator[Receiver[_]]) => {
          ...
          if (TaskContext.get().attemptNumber() == 0) {
            ...
            // 对每个receiver启动一个ReceiverSupervisor
      ...
      // 根据优先调度位置（scheduledLocations）创建RDD并提交到spark
      // 这里只生成了一个没有任何依赖的RDD，外带启动supervisor的func
      // 这里生成RDD的用处，感觉只是为了利用core包的调度方法，将receiver调度到集群节点上执行
      ...
      // 最终将生成的RDD和func提交到sparkContext中（submitJob）
      val future = ssc.sparkContext.submitJob[Receiver[_], Unit, Unit](
        receiverRDD, startReceiverFunc, Seq(0), (_, _) => Unit, ())
      // We will keep restarting the receiver job until ReceiverTracker is stopped
      future.onComplete {
        case Success(_) =>
          // 如果ReceiverTracker没有停止，则这里会重启receiver
          if (!shouldStartReceiver) {
            onReceiverJobFinish(receiverId)
          } else {
