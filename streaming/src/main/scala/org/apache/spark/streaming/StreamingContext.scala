 ...
 * of the context by `stop()` or by an exception.
  *
  * Spark Streaming函数的入口。提供了多种从不同数据源创建DStream的方法。StreamingContext可以由Spark
  * master URL和appName创建，或者由SparkConf配置文件创建，或者由SparkContext创建。关联的SparkContext
  * 可以通过context.sparkContext创建。在创建和转换DStream后，streaming的计算可以使用context.start开始，
  * 使用context.stop停止。
  * context.awaitTermination允许当前线程等待context的停止（使用stop方法或异常）
 */
class StreamingContext private[streaming] (
   ...
   * if the developer wishes to query old data outside the DStream computation).
    *
    * 对每个DStream设置需要保存的生成的RDD的时长。DStream默认情况下只记录RDD有限的时段，之后就释放
    * 给垃圾回收。这个方法允许开发者指定要记录RDD多久（比如开发者想要查询DStream生成的老记录）
    *
      ...
      case INITIALIZED =>
        ...
        StreamingContext.ACTIVATION_LOCK.synchronized {
          ...
          try {
            ...
            // current thread.
            // 在单独的线程中运行scheduler，所以线程内的属性，如call sites和job groups可以被重置，
            // 而不会改变当前线程中的值
