 ...
 * each batch.
  *
  * 管理分配给StreamingContext的executor的类，可以根据streaming的计算统计动态请求或杀死executor。
  * 这个策略与core包中的动态分配策略不同；core包的策略是根据executor的空闲时间来决定是否要释放
  * executor，而微批量模式streaming避免某个executor被闲置太久。所以，idle-ness参数设置为每个批
  * 处理花费的时间。
 *
 ...
 *   is not running a receiver.
  *
  * 在高层面来看，这个类实现的策略如下：
  * 使用StreamingListener接口来获取已完成的批次的处理时间
  * 周期性的计算每个批次平均的处理时间，并与设定的批次间隔比较
  * 如果ratio大于scaling up ratio，则申请更多的executor。每次申请的executor数量由ratio决定
  * 如果ratio小于scaling down ratio，则释放没有运行receiver的executor
 *
