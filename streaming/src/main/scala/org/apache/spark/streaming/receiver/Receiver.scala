...
abstract class Receiver[T](val storageLevel: StorageLevel) extends Serializable {
   ...
   * immediately, and then `onStart()` after a delay.
    *
    * 在receiver启动时系统会调用这个方法。这个方法必须初始化所有获取数据所需要的资源（线程，缓存等）。
    * 这个方法必须是非阻塞的，所以获取数据必须在不同的线程中。获取的数据可以调用store方法保存到spark。
    *
    * 如果在线程启动后发生错误，可以有如下的处理方法：
    * 1 调用reportError方法将错误报告给driver
    * 2 调用stop方法停止获取数据。
    * 3 调用restart方法重启receiver。这个方法会立即调用onStop方法，并延迟一段时间后调用onStart方法
   */
  def onStart(): Unit
   ...
   */
  // receiver对应的supervisor实例在ReceiverTracker.startReceiver方法中创建
  ...
  // receiver所有动作的实际处理器，这个实例会在worker上延迟初始化
