 ...
 * [[org.apache.spark.streaming.dstream.ReceiverInputDStream]] as the parent class.
  *
  * 这个类是所有输入流的基础抽象类。这个类体统了start，stop方法，可以被Spark Streaming调用来启动
  * 或停止获取数据。
  * 输入流是只运行在driver节点的服务/线程来根据数据生成RDD（这个方法不会在worker节点运行），可以
  * 直接继承InputDStream类实现。例如，FileInputDStream，InputDStream子类，通过driver监听HDFS目录，
  * 当有新文件加入时，生成RDD。如果想实现可以在worker节点运行获取数据的类，继承ReceiverInputDStream
  * 作为父类。
 *
