 ...
 * up DStream metadata.
  *
  * 这个类从DStream和checkpointing生成类，并清理DStream元数据
 */
private[streaming]
class JobGenerator(jobScheduler: JobScheduler) extends Logging {
  ...
  private def startFirstTime() {
    ...
    // 初始化graph input stream和output stream
    graph.start(startTime - graph.batchDuration)
    // 启动内部定时器，定时调用eventloop.post方法生成新的GenerateJobs，
    // 从而生成新批次job来执行
  ...
  private def generateJobs(time: Time) {
    ...
    Try {
      // 首先将receiverTracker中未分配的数据块分配给当前批次
      jobScheduler.receiverTracker.allocateBlocksToBatch(time) // allocate received blocks to batch
      // 根据生成的block，生成当前的rdd，最终生成当前时段的job并提交到spark
      graph.generateJobs(time) // generate jobs using allocated block
    } match {
      // 如果成功获取jobSet，则调用JobScheduler.submitJobSet提交jobSet
      case Success(jobs) =>
        // 获取所有输入流的信息
