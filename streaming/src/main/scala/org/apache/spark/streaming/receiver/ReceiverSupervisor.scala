 ...
 * It provides all the necessary interfaces for handling the data received by the receiver.
  *
  * 用来监督在worker上的receiver的类。提供了所有需要的接口来处理来自receiver的数据
  * Receiver的所有方法最终都实际调用ReceiverSupervisor的方法实现
 */
private[streaming] abstract class ReceiverSupervisor(
  ...
  // 将当前receiverSupervisor注册到receiver上
  ...
  /** Restart receiver with delay */
  // 先停止当前的receiver，并向driver请求删除注册
  // 然后再启动receiver，并向driver注册
