 ...
 * it will try reading events from logs in the directory.
  *
  * 记录所有获取到的数据块并将数据块分配给指定批次的类。这个类中的所有操作都可以被保存到
  * wal log中（如果提供了一个检查点目录），所以tracker的状态（接收数据块以及分配给批次的
  * 数据块）可以在driver崩溃后恢复
  *
  * 注意的是当这个类的任何实例以检查点目录创建时，会读取log目录中的全部event
 */
